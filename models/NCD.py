...
class NCDM(CDM):
    ...
    def train(self, train_data, test_data=None, epoch=10, device="cpu", lr=0.002, silence=False):
        ...
        df_epoch = pd.DataFrame(columns=['epoch', 'auc', 'acc', 'name'])
        df_step = pd.DataFrame(columns=['step', 'loss', 'name'])
        for epoch_i in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s" % epoch_i):
                ...
                df_step.loc[len(df_step.index)] = [batch_count + epoch_i * 256, loss.mean().item(), 'NCDMwcognitive']
            ...
            if test_data is not None:
                ...
                df_epoch.loc[len(df_epoch.index)] = [epoch_i, auc, accuracy, 'NCDMwcognitive']
